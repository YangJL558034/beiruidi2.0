...
export function ProductCard({ product, compact = false }: { product: LocalizedProduct; compact?: boolean }) {
  const pathname = usePathname();
  const locale = getLocaleFromPathname(pathname);
  const t = commonText[locale];

  return (
    <article className={`relative overflow-hidden bg-white text-center ${compact ? "min-h-[500px]" : "min-h-[560px]"}`}>
      ...
      <div className="relative z-10 mx-auto flex max-w-xl flex-col items-center px-5 pt-10 sm:pt-12">
        <h2 className="text-[28px] font-semibold leading-tight text-[#1d1d1f] sm:text-[36px]">{product.name}</h2>
        ...
        <div className="mt-4 flex justify-center gap-6 text-[17px]">
          <Link href={`/${locale}/products/${product.slug}`} className="inline-flex items-center text-[#0071e3] hover:underline">
            {t.learnMoreBtn}
            <ChevronRight size={16} />
          </Link>
          <Link href={`/${locale}/contact`} className="inline-flex items-center text-[#0071e3] hover:underline">
            {t.buyBtn}
            <ChevronRight size={16} />
          </Link>
        </div>
