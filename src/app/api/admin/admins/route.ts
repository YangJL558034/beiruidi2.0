...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (body.role !== undefined) {
      ...
      if (!["owner", "editor", "support"].includes(role)) return NextResponse.json({ error: "角色无效。" }, { status: 400 });
      return NextResponse.json({ admin: updateAdminRole(id, role as "owner" | "editor" | "support") });
    }
