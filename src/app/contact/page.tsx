    ...
    <main className="min-h-screen bg-[#f5f5f7] text-[#1d1d1f]">
      ...
      <section className="px-5 py-14 sm:py-20">
        <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-[0.8fr_1.2fr]">
          <aside className="rounded-[8px] bg-white p-7">
            ...
            <div className="mt-8 grid gap-6 text-sm leading-6 text-[#6e6e73]">
              {settings.contactEmail ? (
                <a
                  className="flex gap-3 hover:text-[#0071e3]"
                  href={`mailto:${settings.contactEmail}`}
                >
                  <Mail size={19} className="mt-0.5 shrink-0 text-[#1d1d1f]" />
                  {settings.contactEmail}
                </a>
              ) : null}
              {settings.contactLocation ? (
                <p className="flex gap-3">
                  <MapPin size={19} className="mt-0.5 shrink-0 text-[#1d1d1f]" />
                  {settings.contactLocation}
                </p>
              ) : null}
          ...
          <div className="rounded-[8px] bg-white p-5 sm:p-7">
            <ContactForm />
          </div>
        </div>
      </section>
      <ContentSectionList
        sections={siteContent.contact.sections}
        locale={locale}
      />
      <MetricsStrip content={siteContent.contact} />
      <ResourceSection content={siteContent.contact} locale={locale} />
      <SiteFooter />
    </main>
  );
}
