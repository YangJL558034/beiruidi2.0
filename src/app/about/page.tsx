...
export default function AboutPage() {
  ...
  const section = (id: string) =>
    siteContent.about.sections?.find((item) => item.id === id);
  const values = [
    {
      content: section("company"),
      title: section("company")?.title || t.designLed,
      copy: section("company")?.subtitle || t.designLedCopy,
      icon: CheckCircle2,
    },
    {
      content: section("philosophy"),
      title: section("philosophy")?.title || t.retailReady,
      copy: section("philosophy")?.subtitle || t.retailReadyCopy,
      icon: Factory,
    },
    {
      content: section("cooperation"),
      title: section("cooperation")?.title || t.globalMinded,
      copy: section("cooperation")?.subtitle || t.globalMindedCopy,
      icon: Globe2,
    },
  ];
  ...
  return (
    <main className="min-h-screen bg-[#f5f5f7] text-[#1d1d1f]">
      ...
      <section className="relative grid min-h-[720px] bg-white lg:grid-cols-2">
        ...
        <div className="flex flex-col justify-center px-5 py-16 text-center lg:px-14 lg:text-left">
          <p className="text-[21px] font-semibold">
            {siteContent.about.eyebrow || t.aboutEyebrow}
          </p>
      ...
      <PageMedia
        media={siteContent.about.media}
        className="mx-3 mb-3 h-[360px] rounded-[8px] sm:h-[500px]"
      />
      <section className="px-5 py-16 sm:py-24">
        <div className="mx-auto grid max-w-6xl gap-4 md:grid-cols-3">
          {values.filter((value) => value.content?.visible !== false).map((value) => {
      ...
      <ContentSectionList
        sections={siteContent.about.sections}
        locale={locale}
        excludeIds={["company", "philosophy", "cooperation"]}
      />
      <MetricsStrip content={siteContent.about} />
      <SiteFooter />
    </main>
  );
}
