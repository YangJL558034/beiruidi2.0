...
type ProductSort = "featured" | "name";
    ...
    <article className="group flex h-full min-w-0 flex-col overflow-hidden rounded-[28px] bg-white shadow-[0_10px_35px_rgba(0,0,0,0.06)] ring-1 ring-black/[0.04] transition duration-500 hover:-translate-y-1 hover:shadow-[0_18px_45px_rgba(0,0,0,0.1)]">
      <div className="flex min-w-0 items-start justify-between gap-4 px-6 pb-3 pt-7 sm:px-8 sm:pt-8">
        <div>
          ...
          <h2 className="mt-3 text-[26px] font-semibold tracking-[-0.02em] text-[#1d1d1f] sm:text-[32px]">
            {product.name}
          </h2>
          <p className="mt-2 max-w-[18rem] text-[16px] leading-6 text-[#6e6e73] sm:text-[17px]">
            {product.subtitle}
          </p>
        </div>
      </div>
      ...
      <div className="mt-auto flex flex-col gap-5 px-6 pb-7 pt-5 sm:px-8 sm:pb-8 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="flex w-full items-center gap-3 text-[15px] font-medium xl:w-auto xl:justify-end">
          <button
            type="button"
            onClick={onCompare}
            className={`inline-flex min-h-9 items-center gap-1 rounded-full px-3 text-sm ${selected ? "bg-[#0071e3] text-white" : "bg-[#f5f5f7] text-[#1d1d1f]"}`}
          >
          ...
          <Link
            href={detailHref}
            className="inline-flex shrink-0 items-center gap-1 whitespace-nowrap text-[#0071e3] hover:underline"
          >
            {t.learnMoreBtn}
...
export default function ProductsPage() {
  ...
  const visibleProducts = useMemo(() => {
    const next = products.filter(
      (product) => filter === "all" || product.featured,
    );
    return [...next].sort((a, b) => {
      if (sort === "name")
        return a.name.localeCompare(b.name, locale === "cn" ? "zh-CN" : "en");
      return (
  ...
  const labels =
    locale === "cn"
      ? {
          ...
          support: "\u9700\u8981\u5e2e\u52a9\uff1f",
          supportCopy:
            "\u6211\u4eec\u7684\u56e2\u961f\u53ef\u4ee5\u4e3a\u4ea7\u54c1\u9009\u62e9\u3001\u6279\u53d1\u548c OEM \u9879\u76ee\u63d0\u4f9b\u5efa\u8bae\u3002",
          contact: "\u8054\u7cfb\u9500\u552e",
          empty:
            "\u6682\u65f6\u6ca1\u6709\u7b26\u5408\u6761\u4ef6\u7684\u4ea7\u54c1\u3002",
          loadError: "\u4ea7\u54c1\u6682\u65f6\u65e0\u6cd5\u52a0\u8f7d\uff0c\u8bf7\u7a0d\u540e\u91cd\u8bd5\u3002",
          loading: "\u6b63\u5728\u52a0\u8f7d\u4ea7\u54c1...",
        }
      : {
          ...
          subtitle:
            "A refined choice for everyday mobile charging, in every finish.",
          ...
          loadError: "Products are temporarily unavailable. Please try again.",
          loading: "Loading products...",
        };

  const section = (id: string) =>
    siteContent.products.sections?.find((item) => item.id === id);
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f5f5f7] text-[#1d1d1f]">
      ...
      <Breadcrumbs
        locale={locale}
        items={[{ label: locale === "cn" ? "产品" : "Products" }]}
        className="border-b border-black/[0.06] bg-white"
      />
      <PageNotice content={siteContent.products} />
      ...
      <section className="bg-[#f5f5f7] px-5 pb-14 pt-20 sm:pb-20 sm:pt-28">
        <div className="mx-auto max-w-6xl text-center">
          <p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#6e6e73]">
            {siteContent.products.eyebrow || labels.eyebrow}
          </p>
          <h1 className="mx-auto mt-4 max-w-4xl text-balance text-[48px] font-semibold leading-[1.04] tracking-[-0.045em] sm:text-[76px]">
            {siteContent.products.title ||
              labels.title ||
              fallback.productsTitle}
          </h1>
          <p className="mx-auto mt-5 max-w-2xl text-balance text-[20px] leading-7 text-[#6e6e73] sm:text-[25px]">
            {siteContent.products.subtitle ||
              labels.subtitle ||
              fallback.productsSubtitle}
          </p>
      ...
      <PageMedia
        media={siteContent.products.media}
        className="mx-3 mb-3 h-[300px] rounded-[28px] sm:h-[470px]"
      />

      <section
        id="all-products"
        className="border-y border-black/[0.08] bg-white px-5 py-7 sm:py-8"
      >
        <div className="mx-auto flex max-w-6xl flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#6e6e73]">
              {catalog?.eyebrow || labels.allModels}
            </p>
            <p className="mt-2 text-[20px] font-semibold tracking-[-0.02em]">
              {catalog?.title || labels.allModelsCopy}
            </p>
          </div>
          {catalog?.subtitle ? (
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <label className="flex min-h-11 items-center gap-2 rounded-full border border-black/[0.1] bg-white px-4 text-sm text-[#6e6e73]">
              ...
              <select
                ...
                className="appearance-none bg-transparent pr-5 font-medium text-[#1d1d1f] outline-none"
              >
      ...
      <PageMedia
        media={catalog?.media}
        className="mx-3 mt-3 h-[260px] rounded-[28px] sm:h-[420px]"
      />

      <section className="px-4 py-6 sm:px-6 sm:py-10">
        <div className="mx-auto max-w-6xl">
          {compared.length >= 2 ? (
            <div className="mb-6 overflow-x-auto rounded-md border border-[#0071e3]/20 bg-white p-5">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <p className="font-semibold">
                    {locale === "cn" ? "产品对比" : "Product comparison"}
                  </p>
              ...
              <table className="w-full min-w-[620px] text-sm">
                <thead>
                  <tr>
                    <th className="p-2 text-left text-[#6e6e73]">
                      {locale === "cn" ? "规格" : "Specification"}
                    </th>
                    {compared.map((item) => (
                ...
                <tbody>
                  {[
                    [locale === "cn" ? "颜色" : "Color", "color"],
                    [locale === "cn" ? "容量" : "Capacity", "capacity"],
                    [locale === "cn" ? "输入" : "Input", "input"],
                    [locale === "cn" ? "输出" : "Output", "output"],
                  ].map(([label, key]) => (
                    <tr key={key} className="border-t border-black/[0.06]">
                      <td className="p-2 text-[#6e6e73]">{label}</td>
                      {compared.map((item) => (
          ...
          {loading ? (
            <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
              {[1, 2, 3].map((item) => (
                <div
                  key={item}
                  className="h-[560px] animate-pulse rounded-[28px] bg-white"
                />
              ))}
            </div>
            ...
            <div className="rounded-[28px] bg-white px-6 py-20 text-center text-[#6e6e73]">
              {loadFailed ? labels.loadError : labels.empty}
            </div>
          )}
        </div>
      </section>
      ...
      <section className="bg-white px-5 py-20 sm:py-28">
        <div className="mx-auto max-w-6xl">
          <div className="max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#6e6e73]">
              {guides?.eyebrow || labels.guides}
            </p>
            <h2 className="mt-3 text-[38px] font-semibold leading-[1.08] tracking-[-0.04em] sm:text-[56px]">
              {guides?.title || labels.guidesCopy}
            </h2>
            {guides?.subtitle ? (
          ...
          <PageMedia
            media={guides?.media}
            className="mt-10 h-[260px] rounded-[28px] sm:h-[420px]"
          />
          <div className="mt-12 grid gap-4 md:grid-cols-2">
            <button
              ...
              className="group rounded-[28px] bg-[#f5f5f7] p-7 text-left transition hover:bg-[#ededf0] sm:p-9"
            >
              <Sparkles size={24} strokeWidth={1.7} />
              <PageMedia
                media={daily?.media}
                className="mt-6 h-48 rounded-[20px]"
              />
              <h3 className="mt-16 text-[28px] font-semibold tracking-[-0.02em]">
                {daily?.title || labels.daily}
              </h3>
              <p className="mt-2 max-w-md text-[17px] leading-7 text-[#6e6e73]">
                {daily?.subtitle || labels.dailyCopy}
              </p>
              <span className="mt-6 inline-flex items-center gap-1 text-[15px] font-medium text-[#0071e3]">
                {daily?.primaryLabel || labels.all}
                <ArrowRight
                  size={16}
            ...
            <Link
              href={withLocale(retail?.primaryHref || "/contact", locale)}
              className="group rounded-[28px] bg-[#1d1d1f] p-7 text-left text-white transition hover:bg-[#2c2c2e] sm:p-9"
            >
              ...
              />
              <h3 className="mt-16 text-[28px] font-semibold tracking-[-0.02em]">
                {retail?.title || labels.retail}
              </h3>
              <p className="mt-2 max-w-md text-[17px] leading-7 text-white/70">
                {retail?.subtitle || labels.retailCopy}
              </p>
              <span className="mt-6 inline-flex items-center gap-1 text-[15px] font-medium text-[#66b8ff]">
                {retail?.primaryLabel || labels.contact}
                <ArrowRight
                  size={16}
                  className="transition group-hover:translate-x-1"
                />
      ...
      <section className="bg-[#f5f5f7] px-5 py-20 text-center sm:py-24">
        <PageMedia
          media={help?.media}
          className="mx-auto mb-8 h-[240px] max-w-4xl rounded-[28px] sm:h-[420px]"
        />
        <ShieldCheck className="mx-auto" size={28} strokeWidth={1.6} />
        <h2 className="mt-5 text-[32px] font-semibold tracking-[-0.03em] sm:text-[44px]">
          {help?.title || labels.support}
        </h2>
        <p className="mx-auto mt-3 max-w-xl text-[17px] leading-7 text-[#6e6e73]">
          {help?.subtitle || labels.supportCopy}
        </p>
        <Link
          href={withLocale(help?.primaryHref || "/contact", locale)}
          className="mt-7 inline-flex min-h-11 items-center gap-2 rounded-full bg-[#0071e3] px-5 text-[16px] font-medium text-white hover:bg-[#0077ed]"
        >
          {labels.contact}
          <ArrowRight size={17} />
        </Link>
      ...
      <ContentSectionList
        sections={siteContent.products.sections}
        locale={locale}
        excludeIds={["catalog", "guides", "daily", "retail", "help"]}
      />
      <MetricsStrip content={siteContent.products} />
      <SiteFooter />
    </main>
  );
}
